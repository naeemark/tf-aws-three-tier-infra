...
resource "aws_vpc" "bbeans_vpc" {
  ...
  tags       = merge({ Name = "bbeans-vpc" }, var.tags)
}
...
resource "aws_subnet" "public_subnet_1" {
  ...
  tags                    = merge({ Name = "bbeans-public-subnet-1" }, var.tags)
}
...
resource "aws_subnet" "public_subnet_2" {
  ...
  map_public_ip_on_launch = true
  tags                    = merge({ Name = "bbeans-public-subnet-2" }, var.tags)
}
...
resource "aws_subnet" "private_subnet_1" {
  ...
  tags                    = merge({ Name = "bbeans-private-subnet-1" }, var.tags)
}
...
resource "aws_internet_gateway" "igw" {
  vpc_id = aws_vpc.bbeans_vpc.id
  tags   = merge({ Name = "bbeans-internet-gateway" }, var.tags)
...
resource "aws_route_table" "public_subnet_1_rt" {
  ...
  tags = merge({ Name = "bbeans-public-subnet-route-table" }, var.tags)
}
