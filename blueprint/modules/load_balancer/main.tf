...
resource "aws_lb_target_group" "frontend_tg_80" {
  name     = "frontend-tg-80-${var.tags.Env}"
  port     = 80
  protocol = "HTTP"
  vpc_id   = var.vpc_id
  health_check {
...
resource "aws_lb_target_group" "frontend_tg_81" {
  name     = "frontend-tg-81-${var.tags.Env}"
  port     = 81
  protocol = "HTTP"
  vpc_id   = var.vpc_id
  health_check {
...
resource "aws_lb_listener" "frontend_http_80" {
  ...
  port              = "80"
  protocol          = "HTTP"
  depends_on        = [aws_lb_target_group.frontend_tg_80]
  tags              = merge({ Name = "frontend-listener-http-${var.tags.Env}" }, var.tags)
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.frontend_tg_80.arn
  }
}

resource "aws_lb_listener" "frontend_http_81" {
  load_balancer_arn = aws_lb.alb.arn
  port              = "81"
  protocol          = "HTTP"
  depends_on        = [aws_lb_target_group.frontend_tg_81]
  tags              = merge({ Name = "frontend-listener-http-${var.tags.Env}" }, var.tags)
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.frontend_tg_81.arn
  }
}
